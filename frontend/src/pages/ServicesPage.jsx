...
function ServicesPage() {
  ...
  useEffect(() => {
    const fetchArtisans = async () => {
      try {
        const response = await getArtisans(searchQuery);
            ...
            <div className="flex flex-col gap-10 px-4 py-10 @container">
              ...
              <div className="grid grid-cols-[repeat(auto-fit,minmax(280px,1fr))] gap-4">
                {artisans.map((artisan) => (
                  <div key={artisan.id} className="flex flex-col gap-4 rounded-lg border border-[#e6dddb] bg-white p-6">
                    <div className="flex flex-col gap-2">
                      <h2 className="text-[#181211] text-xl font-bold leading-tight">{artisan.full_name}</h2>
                      <p className="text-[#8a6a60] text-sm font-normal leading-normal">{artisan.location}</p>
                    </div>
                    ...
                    <button
                      ...
                      className="flex min-w-[84px] max-w-[480px] cursor-pointer items-center justify-center overflow-hidden rounded-xl h-10 px-4 bg-[#f35120] text-white text-sm font-bold leading-normal tracking-[0.015em] mt-4"
                    >
