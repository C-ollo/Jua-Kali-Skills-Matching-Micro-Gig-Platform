...
function ClientJobManagement() {
  ...
  const fetchPostedJobs = async () => {
    ...
    try {
      ...
      const clientJobs = response.data.jobs.filter(job => job.client_id === user.id);
  ...
  const handleDeleteJob = async (jobId) => {
    if (window.confirm('Are you sure you want to delete this job?')) {
      try {
        ...
        alert('Failed to delete job. ' + (err.response?.data?.detail || 'Please try again.'));
      }
    }
  };

  if (loading) {
    return <p className="text-brand-text-secondary">Loading your posted jobs...</p>;
  }
  ...
  if (!user || user.user_type !== 'client') {
    return <p className="text-brand-text-secondary">This section is for clients only.</p>;
  }
          ...
          <div className="layout-content-container flex flex-col max-w-[960px] flex-1">
            <div className="flex flex-col gap-10 px-4 py-10 @container">
              ...
              <button
                ...
                className="flex min-w-[84px] max-w-[480px] cursor-pointer items-center justify-center overflow-hidden rounded-xl h-10 px-4 bg-brand-primary text-white text-sm font-bold leading-normal tracking-[0.015em] mb-4"
              >
                <span className="truncate">Post a New Job</span>
              </button>
              {postedJobs.length === 0 ? (
                <p className="text-brand-text-secondary">You haven't posted any jobs yet.</p>
              ) : (
                  ...
                  {postedJobs.map((job) => (
                    <li key={job.id} className="bg-white p-3 rounded-md shadow-sm border border-brand-border">
                      ...
                      <div className="mt-3 space-x-2">
                        <Link
                          to={`/jobs/${job.id}`}
                          className="bg-brand-primary hover:bg-orange-700 text-white font-bold py-1 px-3 rounded-md text-sm transition duration-300"
                        >
                          View Details
                        </Link>
                        <button
